...
async function main() {
  ...
  await sleep(300);
  check('B received trade request', b.events.some((e) => e.type === 'tradeRequest'));
  ...
  await sleep(300);
  check('offers visible to the other side',
  ...
  check('items swapped atomically', aFangs === 1 && bFangs === 2 && aBread === 1,
  ...
  await sleep(300);
  check('B received duel challenge', b.events.some((e) => e.type === 'duelRequest'));
  ...
  for (let i = 0; i < 120 && !ended; i++) {
    const facing = Math.atan2((b.self?.x ?? 0) - (a.self?.x ?? 0), (b.self?.z ?? 0) - (a.self?.z ?? 0));
  ...
  check('duel ended with a winner', !!endEvent && endEvent.winnerName.startsWith('Trader'), JSON.stringify(endEvent));
